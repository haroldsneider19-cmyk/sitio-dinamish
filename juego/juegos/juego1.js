...
const Game1 = (() => {
  ...
  function buildLevel(idx){
    // genera un nivel procedural con dificultad creciente adaptado para verse más grande
    const rnd = mulberry32(1000 + idx*77);
    const groundY = H - 90;
    const platforms = [{x:0, y:groundY, w:1000, h:140}];
    const obstacles = [];
    const collectibles = [];
    let cursorX = 300;
    const segments = 6 + idx*2;
    for(let i=0;i<segments;i++){
      const gap = 110 + rnd()*60;
      const platW = 180 + rnd()*140;
      const platY = groundY - (rnd() > 0.5 ? (80 + rnd()*100) : 0);
      cursorX += gap;
      platforms.push({x:cursorX, y:platY, w:platW, h: (H - platY)});
      if(rnd() > 0.35){
        obstacles.push({
          x: cursorX + platW*0.3 + rnd()*platW*0.3, y: platY - 44, w:44, h:44,
      ...
      for(let c=0;c<2;c++){
        if(rnd() > 0.4){
          collectibles.push({
            x: cursorX + rnd()*platW, y: platY - 70 - rnd()*40, r:16, collected:false,
    ...
    platforms.push({x: cursorX + 40, y: groundY, w: 260, h: 140});
  ...
  function resetPlayer(){
    player = { x: 60, y: H-180, w:56, h:56, vx:0, vy:0, onGround:false, dir:1, anim:0 };
    camX = 0;
  }
  ...
  function update(){
    ...
    for(const p of level.platforms){
      const feet = { x: player.x+8, y: player.y+player.h, w: player.w-16, h: 8 };
  ...
  function draw(){
    ...
    ctx.fillText('🏁', level.goalX-15, level.groundY-10);

    // obstáculos más grandes
    ctx.font = '40px sans-serif';
    for(const o of level.obstacles){
      ctx.fillText(ICONS[o.type]||'⚠️', o.x-18, o.y+34);
    }

    // coleccionables más grandes
    ctx.font = '32px sans-serif';
    for(const c of level.collectibles){
      if(c.collected) continue;
      const bob = Math.sin(Date.now()/300 + c.x) * 6;
      ctx.fillText(ICONS[c.type]||'⭐', c.x-16, c.y+10+bob);
    }
    ...
    for(let i=0;i<lives;i++) ctx.fillText('🐙', 16 + i*34, H-16);
  }
